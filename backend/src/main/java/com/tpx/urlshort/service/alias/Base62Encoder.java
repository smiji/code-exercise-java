...
public class Base62Encoder {
    ...
    private static final char[] ALPHA_NUMERICS_CHARS = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".toCharArray();
    ...
    private static final long TOKEN_THRESHOLD = 3521614606207L; // 62^7
    ...
    public String encode(long number) {
        ...
        if (number > TOKEN_THRESHOLD) {
            String message = String.format("Value %d exceeds maximum capacity for %d chars ( %d )", number, FIXED_SIZE_OF_SHORTENED_URL, TOKEN_THRESHOLD);
            logger.error(message);
            throw new IllegalParametersException(message);
        ...
        while (number > 0) {
            numCharHolder[--placer] = ALPHA_NUMERICS_CHARS[(int) number % ALPHA_NUMERICS_LENGTH];
