...
class UrlControllerTest {
    ...
    void shortenUrl_shouldReturn201AndBody() throws Exception {
        ...
        String expectedShortUrl = "http://localhost:8080/my-custom-alias";
        ...
        mockMvc.perform(post("/api/v1/shorten").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isCreated()).andExpect(jsonPath("$.shortUrl").value(expectedShortUrl)).andExpect(jsonPath("$.actualUrl").value(fullUrl));
    }
    ...
    void shortenUrl_whenAliasTaken_shouldReturn400() throws Exception {
        ...
        String alias = "my-custom-alias";
        UrlRequestDTO request = new UrlRequestDTO(fullUrl, alias);
        Mockito.when(urlService.shortenAndPersistURL(request)).thenThrow(new AliasAlreadyPresentException("The alias -my-custom-alias already exist"));

        mockMvc.perform(post("/api/v1/shorten").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(request))).andExpect(status().isBadRequest()).andExpect(jsonPath("$.status").value(400)).andExpect(jsonPath("$.error").value("Invalid input or alias already taken")).andExpect(jsonPath("$.message").value("The alias -my-custom-alias already exist"));
    }

    @Test
    void redirectToFullUrl_shouldReturn302AndLocationHeader() throws Exception {
        String shortUrl = "http://localhost:8080/my-custom-alias";
        ...
        mockMvc.perform(get("/api/v1/my-custom-alias")).andExpect(status().isFound()).andExpect(header().string("Location", actualUrl));
    }

    @Test
    void redirectToFullUrl_whenAliasMissing_shouldReturn404() throws Exception {
        Mockito.when(urlService.findByAlias("missing-alias")).thenThrow(new ItemNotFoundException("No details found for the alias missing-alias"));
        mockMvc.perform(get("/api/v1/missing-alias")).andExpect(status().isNotFound()).andExpect(jsonPath("$.status").value(404)).andExpect(jsonPath("$.error").value("Alias not found")).andExpect(jsonPath("$.message").value("No details found for the alias missing-alias"));
    }
    ...
    void deleteUrl_whenAliasMissing_shouldReturn404() throws Exception {
        ...
        Mockito.doThrow(new ItemNotFoundException("No details found for the alias not-here")).when(urlService).delete("not-here");
